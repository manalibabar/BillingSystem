...
import static database.DB_Connection.pst;
import static database.DB_Connection.stmt;
...
public class GenerateBill extends javax.swing.JFrame {
   ...
   //Method to get the Service Name
    void getBill_No()
    {
       ...
       /* 
         try {
             String s=billNo.getText();
        int billnumber=Integer.parseInt(s);
     ...
     //Method to Calculate Amount and Grand Total
   ...
   public void insertBillDetails()
    {
        ...
        else
        {
            ...
            try {
             ...
             // pst=con.prepareStatement("insert into Bill_details values(?,?,?,?,?,?,?,?,?,?,?,?)");
    ...
    private void initComponents() {
        ...
        billType.setModel(new javax.swing.DefaultComboBoxModel<>(new String[] { "Choose", "OPD", "IPD", "Sonography", "RECEPT" }));
        ...
        jPanel1.add(jButton1, new org.netbeans.lib.awtextra.AbsoluteConstraints(450, 620, 100, 40));
        ...
        jPanel1.add(jLabel17, new org.netbeans.lib.awtextra.AbsoluteConstraints(10, 0, 120, 40));
        ...
        //to calculate Grand Total
          try {
              if(disType.getSelectedItem().equals("%(per)")){
                //  if(amount.getText()!=null && discount.getText()!=null)
                 // {
    ...
    private void BpatientActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_BpatientActionPerformed
       ...
       /*  try {
             Connection con =getConnection();//accessing database connection from DB_Connection class
            String sql="select Register_No into Bill_details(PRegNo) from Patient where Patient_Name=?";
          ...
          //  stmt=con.createStatement();
            ...
            while(rt.next())
            {
                int Reg_No= rt.getInt("Register_No");
                
                pt.executeUpdate("insert into Bill_details (PRegNo) values('"+Reg_No+"')");    
            }
        ...
        try {
            ...
            while(rs.next())
            {
                Pregno.setText(rs.getString("Register_No"));//set Price to JTextFiled
            }
        }
        catch(NumberFormatException ex)
        {
